...
def save_all_weights(d, g, s, epoch_number, current_loss):
    ...
    save_dir = os.path.join('GAN/SIAMESE/Mse/', '{}{}'.format(now.month, now.day))
...
def train_multiple_outputs(n_images, batch_size, log_dir, epoch_num, critic_updates=5):
    ...
    output_true_batch, output_false_batch, output_zero_batch  = np.ones((batch_size, 1)), -np.ones((batch_size, 1)), np.zeros((batch_size, 1))
    ...
    for epoch in tqdm.tqdm(range(epoch_num)):
        ...
        for index in range(int(x_train.shape[0] / batch_size)):
            ...
            d.trainable = False
            d_on_s_on_g_loss = d_on_s_on_g.train_on_batch([image_blur_batch,image_full_batch], [image_full_batch, output_true_batch, output_zero_batch])
