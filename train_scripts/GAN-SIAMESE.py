...
# input_shape = np.array([336,672,1])
...
def train_multiple_outputs(input_shape,n_images, batch_size, log_dir, epoch_num, critic_updates=5):
    ...
    s = siamese_model(input_shape  = input_shape)
    ...
    # d = multi_gpu_model(d, gpus=2)
    # s = multi_gpu_model(s, gpus=2)
    # d_on_s_on_g = generator_containing_siamese_multiple_inputs_outputs(g, d, s)
    ...
    loss = [MSSIM_loss, wasserstein_loss, wasserstein_loss ]
    ...
    for epoch in tqdm.tqdm(range(epoch_num)):
        ...
        for index in range(int(x_train.shape[0] / batch_size)):
            ...
            image_noisy_batch = x_train[batch_indexes]
            image_full_batch = y_train[batch_indexes]

            generated_images = g.predict(x=image_noisy_batch, batch_size=batch_size)
            ...
            for _ in range(critic_updates):
                ...
                s_loss_noisy = s.train_on_batch([image_noisy_batch,image_full_batch], output_false_batch)
                s_loss = 0.5 * np.add(s_loss_real, s_loss_noisy)

                s_losses.append(s_loss)

            d.trainable = False
            s.trainable = False
            d_on_s_on_g_loss = d_on_s_on_g.train_on_batch([image_noisy_batch,image_full_batch], [image_full_batch, output_true_batch, output_true_batch])
        ...
        print( np.mean(d_on_s_on_g_losses))
        with open('log.txt', 'a+') as f:
            f.write('{} - {} - {} - {}\n'.format(epoch, np.mean(d_losses), np.mean(s_losses), np.mean(d_on_s_on_g_losses)))
...
# x_train, validation_input = np.split(x_train, [33])
# y_train, validation_output = np.split(y_train, [33])
# x_test, y_test = get_data(path_test, train=True)
# x_train, y_train = get_patch_data(path_train, train=True)
